...
class RoadmapController {
  ...
  async generateRoadmap(req, res) {
    ...
    try {
      ...
      const enhancedRoadmap = await this.enhanceRoadmapData(roadmap);
      ...
      // Save to database
      const saveResult = await this.saveRoadmapToDatabase(userId, enhancedRoadmap, profileData);
      ...
      const roadmapStats = this.calculateRoadmapStats(enhancedRoadmap);

      res.json({ 
        success: true, 
        message: 'Enhanced roadmap generated and saved successfully',
        roadmapInfo: roadmapStats
      });
   ...
   */
  async enhanceExistingRoadmap(req, res) {
    ...
    try {
      ...
      const enhancedRoadmap = await this.enhanceRoadmapData(pathData.path_data);
  ...
  async getUserRoadmap(req, res) {
    ...
    try {
      ...
      // Transform the path_data
      const transformedRoadmap = this.transformRoadmapForFrontend(learningPath);
   ...
   */
  async updateModuleCompletion(req, res) {
    ...
    try {
      ...
      const updateResult = this.updateModuleInPathData(pathData, moduleId, isCompleted);
  ...
  /**
   * Enhance roadmap data
   */
  async enhanceRoadmapData(roadmap) {
    let enhancedRoadmap = { ...roadmap };
    
    if (enhancedRoadmap.modules && Array.isArray(enhancedRoadmap.modules)) {
      enhancedRoadmap.modules = this.enhanceModuleData(enhancedRoadmap.modules);
    ...
    if (enhancedRoadmap.phases && Array.isArray(enhancedRoadmap.phases)) {
      enhancedRoadmap.phases = enhancedRoadmap.phases.map(phase => ({
        ...phase,
        modules: phase.modules ? this.enhanceModuleData(phase.modules) : []
      }));
  ...
  /**
   * Enhance individual modules
   */
  enhanceModuleData(modules) {
    return modules.map((module, index) => {
      const enhanced = { ...module };
      
      // Add module description if missing
      if (!enhanced.module_description) {
        enhanced.module_description = this.generateModuleDescription(enhanced.module_name);
      }
      ...
      if (enhanced.resources && Array.isArray(enhanced.resources)) {
        enhanced.resources = enhanced.resources.map((resource, resourceIndex) => {
          if (typeof resource === 'string') {
            return {
              ...
              estimated_time_minutes: this.estimateTimeFromTitle(resource)
            };
          }
          ...
          return {
            ...
            url: resource.url || this.generateResourceUrl(resource.resource_title || resource.title || ''),
        ...
        enhanced.resources = this.generateDefaultResources(enhanced.module_name);
      }
      
      // Add tasks generation
      if (!enhanced.tasks || !Array.isArray(enhanced.tasks) || enhanced.tasks.length === 0) {
        enhanced.tasks = this.generateDefaultTasks(enhanced.module_name, enhanced.module_id || index + 1);
      } else {
      ...
      if (!enhanced.estimated_completion_time_hours && !enhanced.estimated_completion_hours) {
        enhanced.estimated_completion_time_hours = this.calculateEstimatedHours(enhanced.module_name);
      }
   ...
   */
  async saveRoadmapToDatabase(userId, enhancedRoadmap, profileData) {
    try {
      ...
      if (saveResult.error) {
        ...
        return { success: false, error: `Database error: ${saveResult.error.message}` };
      }
  ...
  /**
   * Transform roadmap for frontend
   */
  transformRoadmapForFrontend(learningPath) {
    const pathData = learningPath.path_data;
    ...
    const enhancedModules = allModules.map((module, index) => {
      ...
      return {
        ...
        module_description: module.module_description || module.description || this.generateModuleDescription(module.module_name || module.title),
        ...
        resources: module.resources || this.generateDefaultResources(module.module_name || module.title),
  ...
  /**
   * Update module in path data
   */
  updateModuleInPathData(pathData, moduleId, isCompleted) {
    ...
    if (!moduleFound) {
      return { success: false, error: 'Module not found in roadmap' };
    }
   ...
   */
  calculateRoadmapStats(enhancedRoadmap) {
  ...
  generateModuleDescription(moduleName) {
  ...
  generateDefaultResources(moduleName) {
    ...
    baseResources.push({
      ...
      url: this.generateResourceUrl(`${moduleName} documentation`),
    ...
    baseResources.push({
      ...
      url: this.generateResourceUrl(`${moduleName} tutorial video`),
      resource_type: 'video',
    ...
    baseResources.push({
      ...
      url: this.generateResourceUrl(`${moduleName} guide tutorial`),
  ...
  generateDefaultTasks(moduleName, moduleId) {
    if (!moduleName) return [];
  ...
  generateResourceUrl(resourceTitle) {
  ...
  detectResourceType(resourceTitle) {
    const title = resourceTitle.toLowerCase();
  ...
  calculateEstimatedHours(moduleName) {
